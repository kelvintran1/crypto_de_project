import requests
import pymysql
from eth_wallet_queries import INSERT_WALLET_BALANCE, INSERT_TOKEN_INFO, INSERT_WALLET_HOLDINGS
from crypto_de_project.connect.mysql_config import HOST, PORT, USER, PASSWORD, DB

ETHPLORER_API_KEY = ''
ETHPLORER_URL = 'https://api.ethplorer.io/getAddressInfo/'

# Celebrity: Public address
WALLETS = {
    'Steph Curry': 'REDACTED',
    'Reese Witherspoon': 'REDACTED',
    'Snoop Dogg': 'REDACTED',
    'DJ Marshmello': 'REDACTED',
           }
...
class WalletInfoParser:
    ...
    def get_wallet_info(self):
        balance_url = ETHPLORER_URL + self.wallet_address + '?apiKey=' + ETHPLORER_API_KEY
    ...
    def load_wallet_holding(self):
        ...
        wallet_id = self.cursor.fetchone()
        ...
        self.cursor.execute(INSERT_WALLET_HOLDINGS,
                            (wallet_id, token_id, self.token_info['balance'], self.token_info['balance']))
